 ...
 */
public abstract class ItemQuery<K extends Identifiable & Nameable & Stackable & Actionable & Viewable> extends AbstractQuery<ItemQuery<K>, K, ClientContext>
	 ...
	 */
	@Override
	public ItemQuery<K> id(final Collection<Integer> ids) {
		return select(new Identifiable.Matcher(ids));
	}
	 ...
	 */
	@Override
	public ItemQuery<K> action(final Collection<Pattern> actions) {
		return select(new Actionable.Matcher(actions));
	}
